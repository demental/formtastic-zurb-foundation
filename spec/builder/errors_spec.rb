...
describe 'Formtastic::FormBuilder#errors_on' do
  ...
  before(:each) do
    ...
    @orig_inline_errors      = FormtasticZurbFoundation::FormBuilder.inline_errors
    @orig_inline_error_class = FormtasticZurbFoundation::FormBuilder.default_inline_error_class
    @orig_error_list_class   = FormtasticZurbFoundation::FormBuilder.default_error_list_class
  end

  after(:each) do
    FormtasticZurbFoundation::FormBuilder.inline_errors              = @orig_inline_errors
    FormtasticZurbFoundation::FormBuilder.default_inline_error_class = @orig_inline_error_class
    FormtasticZurbFoundation::FormBuilder.default_error_list_class   = @orig_error_list_class
  end
  ...
  describe 'when there are errors' do
    ...
    # after do
      # FormtasticZurbFoundation::FormBuilder.default_inline_error_class = 'inline-errors'
      # FormtasticZurbFoundation::FormBuilder.default_inline_error_class = 'help-inline'
      # FormtasticZurbFoundation::FormBuilder.default_error_list_class = 'errors'
    # end

    it 'should render a paragraph with the errors joined into a sentence when inline_errors config is :sentence' do
      FormtasticZurbFoundation::FormBuilder.inline_errors = :sentence
    ...
    it 'should render a paragraph with a overridden default class' do
      FormtasticZurbFoundation::FormBuilder.default_inline_error_class = 'better-errors'
      concat(semantic_form_for(@new_post) do |builder|
        builder.input(:title)
      end)
    ...
    it 'should render an unordered list with the class errors when inline_errors config is :list' do
      FormtasticZurbFoundation::FormBuilder.inline_errors = :list
      concat(semantic_form_for(@new_post) do |builder|
        builder.input(:title)
      end)
    ...
    it 'should render an unordered list with the class overridden default class' do
      FormtasticZurbFoundation::FormBuilder.inline_errors = :list
      FormtasticZurbFoundation::FormBuilder.default_error_list_class = "better-errors"
      concat(semantic_form_for(@new_post) do |builder|
        builder.input :title
      end)
    ...
    it 'should render a paragraph with the first error when inline_errors config is :first' do
      FormtasticZurbFoundation::FormBuilder.inline_errors = :first
      concat(semantic_form_for(@new_post) do |builder|
        builder.input :title
      end)
    ...
    it 'should return nil when inline_errors config is :none' do
      FormtasticZurbFoundation::FormBuilder.inline_errors = :none
      concat(semantic_form_for(@new_post) do |builder|
        builder.input :title
      end)
  ...
  describe 'when there are no errors (empty array)' do
    ...
    it 'should return nil when inline_errors config is :sentence, :list or :none' do
      with_deprecation_silenced do
        [:sentence, :list, :none].each do |config|
          FormtasticZurbFoundation::FormBuilder.inline_errors = config
          semantic_form_for(@new_post) do |builder|
            builder.errors_on(:title).should be_nil
          end
        end
